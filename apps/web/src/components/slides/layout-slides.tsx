	...
	return (
		<section>
			...
			>
				...
				{subtitle && (
					<p className="mt-4 text-center text-2xl text-gray-300">
						{subtitle}
					</p>
				)}
...
export function TwoColumnSlide({
	...
	return (
		<section {...bgProps}>
			...
			>
				{title && (
					<h2 className="mb-8 text-left text-white">{title}</h2>
				)}
				<div className="flex items-start justify-between gap-8">
					<div className={`${widthClasses[leftWidth].left} text-left`}>
						{left}
					</div>
					<div className={`${widthClasses[leftWidth].right} text-left`}>
						{right}
					</div>
				</div>
			</div>
			...
			>
				...
				<div className="flex w-full max-w-5xl flex-col gap-6">
					{steps.map((step, i) => (
						<div
							key={`step-${step.title}`}
							className="flex items-center gap-6 fragment"
						>
...
export function GradientCoverSlide({
	...
	return (
		<section>
			<div
				className={`${gradientThemes[theme]} flex h-full flex-col justify-between px-16 py-20`}
			>
				<div className="flex flex-1 flex-col items-start justify-center">
					{event && (
						<span className="slide-badge slide-badge-blue mb-6">
							{event}
						</span>
					)}
					...
					{subtitle && (
						<p className="mt-4 text-left text-2xl text-gray-300">
							{subtitle}
						</p>
					)}
				</div>
	...
	return (
		<section>
			...
			>
				...
				<div
					className={`grid ${columnClasses[columns]} w-full max-w-5xl gap-6`}
				>
					{cards.map((card) => (
